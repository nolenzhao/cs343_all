...
variant<double, Ex1, Ex2> rtn2( double i ) {
    ...
    variant<double, Ex1> res = rtn1(i);

    if(holds_alternative<Ex1>(res)){
        return get<Ex1>(res);
    }
	return get<double>(res) + Rand();
}
variant<double, Ex1, Ex2, Ex3> rtn3( double i ) {
    if ( Rand() % eperiod == 0 ) {
        return Ex3{Rand()};
    }	// replace
    variant<double, Ex1, Ex2> res = rtn2(i);

    if(holds_alternative<Ex1>(res)){
        return get<Ex1>(res);
    }
    if(holds_alternative<Ex2>(res)){
        return get<Ex2>(res);
    }
...
int main( int argc, char * argv[] ) {
	...
	for ( int i = 0; i < times; i += 1 ) {
        variant<double, Ex1, Ex2, Ex3> res = rtn3(i);
        ...
        if (holds_alternative<Ex1>(res)){
            ev1 += get<Ex1>(res).code;
            ec1 += 1;
        }
        if (holds_alternative<Ex2>(res)){
            ev2 += get<Ex2>(res).code;
            ec2 += 1;
        }
        if (holds_alternative<Ex3>(res)){
            ev3 += get<Ex3>(res).code;
            ec3 += 1;
        }
