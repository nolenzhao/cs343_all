...
long int Ackermann( long int m, long int n, long int depth ) {
    // Need to save the current env before env is changed again so we take a snapshot of the caller's env in the case we need to longjmp back to it
    jmp_buf prev_env;
    memcpy(prev_env, env, sizeof(jmp_buf));
    calls += 1;
    if ( m == 0 ) {
        if ( rand() % eperiod <= 2 ) { PRINT( T t; ) excepts += 1; longjmp(prev_env, 1); } // replace
        return n + 1;
    } // if
	if ( n == 0 ) {
        jmp_buf tmp_env; 
        if(setjmp(tmp_env) != 0){
            // error handle on non-zero return
			PRINT( cout << " depth " << depth << " E1 " << m << " " << n << " |" );
			if ( rand() % eperiod <= 3 ) { PRINT( T t; ) excepts += 1; longjmp(prev_env, 1); } // replace
        ...
        if (setjmp(tmp_env) != 0){
			...
			if ( rand() % eperiod == 0 ) { PRINT( T t; ) excepts += 1; longjmp(prev_env, 1); } // replace
            PRINT( cout << " E2X " << m << " " << n << endl );
        }
        else{
            // need to set env on first call, then reinitialize it since it may have been changed by the inner call
            env = &tmp_env;
            long int inner = Ackermann( m, n - 1, depth + 1);
            env = &tmp_env;
...
int main( int argc, char * argv[] ) {
    ...
    jmp_buf tmp_env;                                    // Create an env variable to hold the current context
    // first invocation will always be zero
    if(setjmp(tmp_env) != 0){
        // error handling given non-zero return will indicate coming from a longjmp
        PRINT( cout << "E3" << endl);
    }
    else{
        env = &tmp_env;                                // Point to the current env with the global env so the ensuing function call can longjmp back here (do this after setjmp)
